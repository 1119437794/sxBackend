...
define(function(require, exports, module) {
    ...
    if(!SearchResListSubmitAct.hasOwnProperty("info")) {
      Object.defineProperty(SearchResListSubmitAct, "info", {
        set: function (info) {
          var is_company = info["isCompany"];
          //$(".search-key").val($target.text());
          $("#static-risk-result").hide();
          var s = {
            target: {
              style: {
                text: info.companyName.trim()
              }
            }
          };

          if (is_company == "rect") {
            s.target.isCompany = true;
            node_mouse_click(s, "rect");
            zr_node_click = false;
          } else {
            s.target.isCompany = false;
    ...
    //debugger;
      ...
      DynamicPicCompanyNode.companyName = name;
  ...
  // $('#btn_hidden_word')
    ...
    if( !window.BTN__HIDE__WORD )  return ;

    window.BTN__HIDE__WORD = false;
